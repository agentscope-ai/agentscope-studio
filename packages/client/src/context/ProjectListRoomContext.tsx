...
export function ProjectListRoomContextProvider({ children }: Props) {
	...
	const maxPollingCount = 1000; // 最大轮询次数
	const pollingInterval = 5000; // 5秒轮询间隔
	 ...
	 */
	const getProjects = useCallback(async (params: TableRequestParams) => {
		try {
	 ...
	 */
	const fetchTableData = useCallback(async (params: TableRequestParams) => {
	...
	const silentRefresh = useCallback(async () => {
		try {
			...
			const res = await getProjects({
				pagination: { page: currentState.pagination.current, pageSize: currentState.pagination.pageSize },
				filters: currentState.searchText ? { project: currentState.searchText } : undefined,
				sort: currentState.sortField ? { field: currentState.sortField, order: currentState.sortOrder || 'asc' } : undefined,
			});
	...
	const onTableChange = useCallback((
		...
		const requestParams = {
			...
			sort: nextSortField ? { field: nextSortField, order: nextSortOrder || 'asc' } : undefined,
		};
